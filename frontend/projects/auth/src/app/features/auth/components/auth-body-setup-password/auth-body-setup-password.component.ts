...
/**
 * Auth Body Setup Password Component
 * For users who don't have a password (Firebase migrated, social login, OTP-only)
 */
@Component({
  selector: 'auth-body-setup-password',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, LoaderSpinnerComponent],
  template: `
    <div class="flex flex-col gap-6">
      <div class="text-center">
        <h2 class="text-xl font-semibold text-primary">Set up your password</h2>
        <p class="mt-2 text-sm text-gray-500 leading-relaxed">
          Create a password to sign in with email and password in the future.
        </p>
      </div>
        ...
        <form [formGroup]="formGroup" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
          ...
          @if (allowSkip) {
            <button
              type="button"
              (click)="onSkip()"
              class="text-sm text-gray-500 hover:text-primary hover:underline self-center"
            >
              Skip for now
            </button>
          }
...
export class AuthBodySetupPasswordComponent {
  ...
  async onSubmit(): Promise<void> {
    if (!this.password?.value || !this.email) {
      return;
    }
    ...
    try {
      await this.authService.setupPassword(
        this.email,
        this.password.value,
      ...
      if (isAuthError) {
        ...
        this.errorMessage = err?.message || 'Failed to set password. Please try again.';
      }
    } finally {
