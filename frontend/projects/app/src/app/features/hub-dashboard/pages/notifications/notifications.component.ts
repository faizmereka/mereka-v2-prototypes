...
import { HubNotificationSettingsComponent } from '../settings/notifications/notifications.component';
import {
  HubNotificationService,
  type HubNotification,
} from '../../services/hub-notification.service';

@Component({
  selector: 'app-hub-notifications',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class HubNotificationsComponent implements OnInit {
  ...
  markAsRead(notification: HubNotification) {
  ...
  deleteNotification(event: Event, notification: HubNotification) {
  ...
  getNotificationClasses(templateId?: string): string {
    ...
    const lowerTemplateId = templateId.toLowerCase();
    if (lowerTemplateId.includes('confirmed') || lowerTemplateId.includes('approved'))
